...
def crab_spectrum(energy_gev: float) -> float:
    ...
    """
    return 3.39e-11 * pow(energy_gev / 1000.0, -2.51 - 0.21 * np.log10(energy_gev / 1000.0))
...
class MAGICPerformanceData:
    ...
    def get_performance(zenith: str) -> Tuple[np.ndarray, np.ndarray, np.ndarray, bool]:
        ...
        if zenith == 'low':
            ...
            crabrate[:] = [0.3795, 2.5808, 2.8257, 1.6654, 1.3289, 1.2871, 0.8834,
                          0.7045, 0.3908, 0.1963, 0.089, 0.0368, 0.0]
            bgdrate[:] = [0.88870e+00, 1.6505e+00, 5.7688e-01, 5.0804e-02, 2.0521e-02,
                         2.1718e-02, 5.6106e-03, 3.9491e-03, 3.2053e-03, 1.8074e-03,
                         7.3253e-04, 1.1352e-05, 0.0]
            is_mc = True
            
        elif zenith == 'mid':
            # Aleksic et al 2016 data (30-45° zenith angle)
            crabrate[:] = [0.0, 0.404836, 3.17608, 2.67108, 2.86307, 1.76124, 1.43988,
                          0.944385, 0.673335, 0.316263, 0.200331, 0.0991222, 0.0289831]
            bgdrate[:] = [1.67777, 2.91732, 2.91988, 0.680319, 0.0921932, 0.0203476,
                         0.00893356, 0.00971799, 0.00234261, 0.000665438, 0.00138165,
                         0.00147903, 0.000870903]
            
        elif zenith == 'high':
            # High zenith angle data (~55-62°)
            crabrate[:] = [0.0, 0.0, 0.0, 0.0, 0.503462, 1.60232, 2.26558, 0.928094,
                          0.698335, 0.305662, 0.173859, 0.083892, 0.069938]
            bgdrate[:] = [0.0, 0.0, 0.0, 0.0, 0.750815, 0.597588, 0.564753, 0.089775,
                         0.0568584, 0.00954936, 0.00344762, 0.00147755, 0.00229841]
            is_mc = False
        else:
            raise ValueError(f"Unknown zenith angle: {zenith}. Use 'low', 'mid', or 'high'.")
...
def build_spectrum_function(
    ...
    if flux_norm_tev < 0:
        raise ValueError(f"Flux normalization must be non-negative, got {flux_norm_tev}")
...
def build_spectrum_from_catalog(metadata: Dict[str, Any]) -> Optional[Callable[[float], float]]:
    """
...
def run_mss_simulation(
    ...
    if observation_time_hours <= 0:
        raise ValueError(f"observation_time_hours must be positive, got {observation_time_hours}")
    if psf_deg < 0 or extension_deg < 0:
        raise ValueError("PSF and extension must be non-negative")
    if extension_deg > 1.0:
        raise ValueError("Extension must be < 1 degree (not implemented for large extensions)")
    ...
    crabrate, bgdrate, enbins, is_mc = MAGICPerformanceData.get_performance(zenith_angle)
    ...
    for i in range(npoints):
        ...
        nexc = crabrate[i] * obs_time_minutes * (source_flux_integral / crab_flux_integral)
        
        # Calculate background events (with extension correction)
        noff = bgdrate[i] * obs_time_minutes
        noff *= (psf_deg * psf_deg + extension_deg * extension_deg) / (psf_deg * psf_deg)
        ...
        # Detection criterion
        is_detected = (sigma >= 5.0 and nexc / noff > min_sbr and nexc > min_events) if noff > 0 else False
    ...
    if nexc_all > 0.01 and noff_all > 0.001:
        ...
        aggregate_sigma = total_sigma / np.sqrt(len(spectral_points)) if len(spectral_points) > 0 else 0.0
    ...
    return {
        ...
        'energy_bins': {
            ...
            'centers_gev': [float(np.sqrt(enbins[i] * enbins[i + 1])) for i in range(npoints)],
        },
        ...
        'aggregate_stats': {
            ...
            'total_sbr_percent': (nexc_all / noff_all * 100.0) if noff_all > 0 else np.nan,
        },
        'metadata': {
