...
def Assumed(x):
    ...
    return (
        2.0e-11 * pow(x / 1000.0, -1.99) * np.exp(-x / 100)
...
redshift = (
    -1
)  # redshift of the source (for the EBL absorption), if -1 then no absorption
...
eplotmax = 20.0e3  # [GeV] x plot range
yplotmin = 1.0e-14  # [TeV cm^-2 s^-1] y plot range
yplotmax = 1.0e-9  # [TeV cm^-2 s^-1] y plot range
...
# return 0
    ...
    for i, e1, e2 in reversed(list(zip(range(0, len(enbins)), enbins, enbins[1:]))):
        ...
        if nexc > 0.01:
            if pulsarmode:
                ...
                sigma = SignificanceLiMa(nexc + noff, noff * numoff, 1.0 / numoff)
        ...
        if nexc_all > 0.01:
            if pulsarmode:
                sigma_all = SignificanceLiMa(
                    nexc_all + noff_all * pulsarOnOffRatio, noff_all, pulsarOnOffRatio
                )
            else:
                sigma_all = SignificanceLiMa(
    ...
    plt.plot(x, 1.0e-6 * x * x * Crab(x), "0.3", label="Crab (Aleksic et al 2016)")
    if redshift > 0:
        plt.plot(
            x,
            1.0e-6 * x * x * FluxObs(redshift, x, Assumed(x)),
            "limegreen",
            label=srcname + " (Assumed, z={0:.2f})".format(redshift),
        )
    else:
        plt.plot(
            x, 1.0e-6 * x * x * Assumed(x), "limegreen", label=srcname + " (Assumed)"
        )
    if len(en) > 0:
    ...
    if drawsigma and (best_int_sigma > 0):
        ax.set(
            title="$\sigma$ (>{0:.1f} GeV) = {1:.2f}".format(best_int_e, best_int_sigma)
        )
